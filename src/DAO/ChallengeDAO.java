...
public class ChallengeDAO {
	...
	public List<ChallengeDTO> selectAll(String giveortake) throws SQLException, Exception {
		String sql = "select * from challenge wehre giveortake=?";
		List<ChallengeDTO> list = new ArrayList<>();
		ChallengeDTO cdto = null;
		try (Connection conn = getConnection();
			...
			try(ResultSet rs = pstat.executeQuery();) {
				while (rs.next()) {
					...
					int setTotal_participate = rs.getInt(7);
					String file_path = rs.getString(8);

					cdto = new ChallengeDTO(seq, title, content, start_date, end_date, end, setTotal_participate,
							file_path);
					list.add(cdto);
				}
	...
	public String getPageNavi(int currentPage) throws Exception {
		...
		if (needPrev) {
			sb.append("<a href='list.adboard?currentPage=" + (startNavi - 1) + "'> < </a>");
		}
